...
@section('title', 'Reset Password - Travaiq')
...
<div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-white to-indigo-50 py-8 px-2">
    <div class="w-full max-w-5xl bg-white rounded-3xl shadow-2xl flex flex-col md:flex-row overflow-hidden">
        ...
        <div class="w-full md:w-1/2 flex flex-col justify-center p-8 md:p-16">
            <h2 class="text-3xl font-bold text-gray-900 mb-2 flex items-center justify-center gap-2">Reset  password</h2>
            ...
            <form id="form-login" action="{{ route('resetpasswordsubmit') }}" method="POST" class="space-y-6">
                @csrf
				<input type="hidden" name="token" value="{{ $token ?? old('token') }}">
               
                <div class="relative">
                  
                    <input id="login-email" type="hidden" value="{{ $email }}" name="email" type="email" autocomplete="email" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-primary focus:border-primary text-lg px-4 py-3" placeholder="you@email.com">
                </div>
				...
				<div class="relative">
                    <label for="register-password" class="block text-base font-medium text-gray-700">Confirm Password</label>
                    <div class="relative">
                        <input id="register-password" name="password_confirmation" type="password" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-primary focus:border-primary text-lg px-4 py-3 pr-12" placeholder="Create a password">
                        <button type="button" class="absolute inset-y-0 right-0 pr-3 flex items-center password-toggle" data-target="register-password">
                            <svg class="h-5 w-5 text-gray-400 password-eye" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
